...
def pred_wc_old(sql_i, s_wc):
    ...
    for b, sql_i1 in enumerate(sql_i):
        ...
        pr_wc1 = argsort(-s_wc1.datas.cpu().numpy())[:wn]
        pr_wc1.sort()
...
def pred_wc(wn, s_wc):
    ...
    for b, wn1 in enumerate(wn):
        ...
        pr_wc1 = argsort(-s_wc1.datas.cpu().numpy())[:wn1]
        pr_wc1.sort()
...
def pred_wc_sorted_by_prob(s_wc):
    ...
    for b in range(bS):
        s_wc1 = s_wc[b]
        pr_wc1 = argsort(-s_wc1.datas.cpu().numpy())
        pr_wc.append(list(pr_wc1))
    return pr_wc
...
def pred_wo(wn, s_wo):
    ...
    for b, pr_wo_a1 in enumerate(pr_wo_a):
        wn1 = wn[b]
        pr_wo.append(list(pr_wo_a1.datas.cpu().numpy()[:wn1]))
...
def get_wemb_roberta(roberta_config, model_roberta, tokenizer, nlu_t, hds, max_seq_length, num_out_layers_n=1,
                     num_out_layers_h=1):
    '''
    ...
    wemb_h = get_wemb_h(i_headers, l_hpu, l_hs, roberta_config.hidden_size, roberta_config.num_hidden_layers,
                        all_encoder_layer,
                        num_out_layers_h)
...
def get_roberta_output(model_roberta, tokenizer, nlu_t, headers, max_seq_length):
    ...
    outputs = model_roberta(input_ids=all_input_ids, attention_mask=all_input_mask,
                                                output_hidden_states=True)
    all_encoder_layer = list(outputs.hidden_states)
