...
from seq2seq_internal_functions import *
...
class SCP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_sc=False,
        ...
        # Encode
        mL_n = max(l_n)
        mk = max(len(k) for k in knowledge) if knowledge else 0
        mk = max(mk, mL_n)
        ...
        knowledge = [k + [0] * (mk - len(k)) for k in knowledge]
        knowledge = torch.tensor(knowledge).unsqueeze(-1)

        feature = torch.zeros(bS, mk, self.question_knowledge_dim).scatter_(dim=-1,
                                                                              index=knowledge,
                                                                              value=1).to(device)
        if mk > mL_n:
            wenc_n = torch.nn.functional.pad(wenc_n, (0, 0, 0, mk - mL_n))
        ...
        if show_p_sc:
            ...
            for i_h in range(l_hs[0]):
                ...
                plot(p_n[0][i_h][:].datas.numpy() - i_h, '--' + _symbol[color_idx] + _color[color_idx], ms=7)
...
class SAP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, pr_sc, show_p_sa=False,
        ...
        wenc_n = encode(self.enc_n, wemb_n, l_n,
                        ...
                        hc0=None,
                        last_only=False)  # [b, n, dim]
        knowledge = [k + (mk - len(k)) * [0] for k in knowledge]
        knowledge = torch.tensor(knowledge).unsqueeze(-1)
...
class WNP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wn=False,
        ...
        if show_p_wn:
            ...
            subplot(7,2,5)
            cla()
            plot(p_h[0].datas.numpy(), '--rs', ms=7)
        ...
        if show_p_wn:
            ...
            subplot(7,2,6)
            cla()
            plot(p_n[0].datas.numpy(), '--rs', ms=7)
            title('wn: nlu_weight')
            grid(True)
            fig.canvas.draw()
...
class WCP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wc, penalty=True, predict_select_column=None,
        ...
        if show_p_wc:
            ...
            for i_h in range(l_hs[0]):
                color_idx = i_h % len(_color)
                plot(p[0][i_h][:].datas.numpy() - i_h, '--' + _symbol[color_idx] + _color[color_idx], ms=7)
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn, wc, wenc_n=None, show_p_wo=False,
        ...
        if show_p_wo:
            ...
            for i_wn in range(self.mL_w):
                ...
                plot(p[0][i_wn][:].datas.numpy() - i_wn, '--' + _symbol[color_idx] + _color[color_idx], ms=7)
...
class WVP_se(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn, wc, wo, wenc_n=None, show_p_wv=False,
        ...
        # wenc_n = [bS, mL, 100]
        # vec2 = [bS, 4, mL, 400]
        vec1e = vec.unsqueeze(2).expand(-1,-1, mk, -1) # [bS, 4, 1, 300]  -> [bS, 4, mL, 300]
