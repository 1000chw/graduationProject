...
# BERT
from transformers import RobertaModel, RobertaTokenizer, RobertaConfig, AutoConfig, AutoModel
...
os.environ['CORENLP_HOME'] = '/Users/1000chw/stanford-corenlp-4.5.7'
...
def get_models(args, RoBERTa_PT_PATH, trained=False, path_model_roberta=None, path_model=None):
    ...
    if trained:
        ...
        if torch.cuda.is_available():
            ...
            res = torch.load(path_model_roberta, map_location='cpu')
...
def infer(nlu1,
          ...
          model, model_roberta, tokenizer, bert_config, max_seq_length, num_target_layers,
    ...
    return pr_sql_q, pr_ans


import corenlp
client = corenlp.CoreNLPClient(be_quiet=False, annotators='tokenize,ssplit'.split(','), timeout=100000000000, memory='6G')
...
model, model_roberta, tokenizer, bert_config, args = 0,0,0,0,0
def main():
    global model, model_roberta, tokenizer, bert_config, args
    parser = argparse.ArgumentParser()
    args = construct_hyper_param(parser)
    model, model_roberta, tokenizer, bert_config = get_models(
        args,
        BERT_PT_PATH,
        trained=True,
        path_model_roberta=path_model_roberta,
...
origins = [
    ...
    "http://127.0.0.1:5173",  # 로컬 IP로 접근하는 경우
]
...
def execute(request):
    try:
        ...
        db_name = request.dbInfo['database']
        data_table = load_jsonl('./data_and_model/train.tables.jsonl')
        table_name = request.dbInfo['table']
        parser = argparse.ArgumentParser()
        args = construct_hyper_param(parser)
        model, model_roberta, tokenizer, bert_config = get_models(
            args,
        ...
        print(model, model_roberta, tokenizer)
        return infer(
            nlu1,
            table_name, data_table, path_db, db_name,
            model, model_roberta, tokenizer, bert_config, max_seq_length=args.max_seq_length,
...
if __name__ == "__main__":
    ...
    try:
        main()
        uvicorn.run("backend:app", host="127.0.0.1", port=8000, reload=True, timeout_keep_alive=10000000)
    except Exception as e:
