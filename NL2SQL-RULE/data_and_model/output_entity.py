...
def get_loader_wikisql(data_train, data_dev, bS, shuffle_train=True, shuffle_dev=False):
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=4,
        collate_fn=cfn  # now dictionary values are not merged!
    )
...
def load_wikisql_data(path_wikisql, mode='train', toy_model=False, toy_size=10, no_hs_tok=False, aug=False):
    ...
    print(path_wikisql)
    path_sql = os.path.join(path_wikisql, 'new_ko_' + mode+'_knowledge.jsonl')
    if no_hs_tok:
...
def get_data(path_wikisql, args):
    ...
    train_data, train_table, dev_data, dev_table, _, _ = load_wikisql(path_wikisql,
                                                                      ...
                                                                      no_w2i=True,
                                                                      no_hs_tok=True)
    train_loader, dev_loader = get_loader_wikisql(train_data[:800], dev_data[:800], args["batch_size"], shuffle_train=True)

    return train_data[:800], train_table, dev_data[:800], dev_table, train_loader, dev_loader
