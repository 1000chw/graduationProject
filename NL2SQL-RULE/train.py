...
os.environ['CORENLP_HOME'] = '/data/a2019102233/stanford-corenlp-4.5.7'
...
# BERT
from transformers import RobertaModel, RobertaTokenizer, RobertaConfig, AutoTokenizer, AutoModel, AutoConfig
...
def get_models(args, RoBERTa_PT_PATH, trained=False, path_model_roberta=None, path_model=None):
    ...
    if trained:
        ...
        if torch.cuda.is_available():
            res = torch.load(path_model_roberta)
        else:
            res = torch.load(path_model_roberta, map_location='cpu', weights_only=False)
...
def train(train_loader, train_table, model, model_roberta, opt, bert_config, tokenizer,
    ...
    for iB, t in enumerate(tqdm(train_loader, desc="Training Progress", total=total)):
        cnt += len(t)
...
def test(data_loader, data_table, model, model_roberta, bert_config, tokenizer,
         max_seq_length,
...
def infer(nlu1,
    ...
    if show_answer_only:
        ...
        print(f'{hds}')
        if show_table:
            print(engine.show_table('table'+table_name.replace('-', '_')))
        print(f'nlu: {nlu}')
...
if __name__ == '__main__':
    ...
    ## 2. Paths
    path_h = 'C:\\Users\\1000c\\Desktop\\graduationProject\\NL2SQL-RULE\\data_and_model'  # '/home/wonseok'
    path_wikisql = path_h  # os.path.join(path_h, 'data', 'wikisql_tok')
    BERT_PT_PATH = './'
    ...
    ## 4. Build & Load models
    if not args.trained:
        model, model_roberta, tokenizer, bert_config = get_models(args, BERT_PT_PATH)
    else:
    ...
    ## 5. Get optimizers
    if args.do_train:
        opt, opt_bert = get_opt(model, model_roberta, args.fine_tune)
        ...
        for epoch in range(r):
            ...
            if acc_lx_t > acc_lx_t_best:
                ...
                state = {'model_roberta': model_roberta.state_dict()}
                torch.save(state, os.path.join('.', 'model_roberta_best.pt'))
    ...
    #
    #     nlu1 = "who is the manufacturer for the order year 1998?"
